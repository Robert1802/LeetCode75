...
{
    int greatestValue = candies.Max();
    int[] candiesPlus = new int[candies.Length];
    List<bool> greatestAmongKids = new();

    for (int i = 0; i < candies.Length; i++)
    {
        candiesPlus[i] = candies[i] + extraCandies;
        if (candiesPlus[i] >= greatestValue) greatestAmongKids.Add(true); else greatestAmongKids.Add(false);
    }
